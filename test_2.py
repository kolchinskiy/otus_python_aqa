...
@pytest.mark.parametrize("city_url, expected_city, pages", [
    ('san_diego', 'San Diego', '3')
])
def test_six(base_url_brewery, city_url, expected_city, pages):
    res = requests.get(base_url_brewery + '/v1/breweries?by_city=' + city_url + '&per_page=' + pages)
    for i in range(int(pages)):
        assert res.json()[i].get('city') == expected_city
